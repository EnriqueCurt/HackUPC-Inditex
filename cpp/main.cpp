...
int main(int argc, char* argv[]) {
    ...
    std::string initialCsv = "data/silo-semi-empty.csv";
    bool usarABC = true;

    int historySize = 250000;
    int historyDestinos = 120;
    double historySkew = 1.15;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--history-size" && i + 1 < argc) {
            historySize = std::stoi(argv[++i]);
        } else if (arg == "--history-destinos" && i + 1 < argc) {
            historyDestinos = std::stoi(argv[++i]);
        } else if (arg == "--history-skew" && i + 1 < argc) {
            historySkew = std::stod(argv[++i]);
        }
    ...
    std::cout << "-> Zipf Skew: " << historySkew << " (" << historyDestinos << " destinos)\n";
    ...
    // ==================================================================
    // 2.5 GENERACIÓN DE DEMANDA (REAL VS SINTÉTICA)
    // ==================================================================
    
    std::vector<std::string> bomboDestinosCSV;
    std::vector<std::string> listaDestinosSinteticos;
    std::discrete_distribution<int> distribucionSintetica;
    std::mt19937 generadorRandom(seed);

    if (historySkew <= 0.0) {
        std::cout << "-> Modo Demanda: REAL (Leyendo historico_pedidos.csv)\n";
        miSilo.loadHistory("data/historico_pedidos.csv");

        // Cargamos el bombo para el bucle
        std::ifstream fileHist("data/historico_pedidos.csv");
        ...
        while (std::getline(fileHist, lineaHist)) {
            ...
            if (!lineaHist.empty()) bomboDestinosCSV.push_back(lineaHist);
        }
    } else {
        std::cout << "-> Modo Demanda: SINTETICA (Zipf Skew: " << historySkew << ")\n";
        std::map<std::string, double> zipfFrequencies;
        std::vector<double> pesosZipf;

        for (int i = 0; i < historyDestinos; ++i) {
            char buffer[10];
            snprintf(buffer, sizeof(buffer), "DEST%04d", i);
            std::string destStr = buffer;
            listaDestinosSinteticos.push_back(destStr);

            double peso = 1.0 / std::pow(i + 1, historySkew);
            pesosZipf.push_back(peso);
            zipfFrequencies[destStr] = peso;
        }
        miSilo.setDemandFrequencies(zipfFrequencies);
        distribucionSintetica = std::discrete_distribution<int>(pesosZipf.begin(), pesosZipf.end());
    }
    ...
    while (globalClock < tiempoMaximo) {
        ...
        // EVENTO A: Llegada de cajas nuevas 
        while (globalClock >= nextArrivalTime && cajasEntradasRealizadas < totalCajasNuevas) {
            ...
            // Decidimos de dónde sacar el destino según el modo
            std::string dest;
            if (historySkew <= 0.0) {
                // Modo CSV Real
                dest = bomboDestinosCSV[rand() % bomboDestinosCSV.size()];
            } else {
                // Modo Matemático Zipf
                int indiceElegido = distribucionSintetica(generadorRandom);
                dest = listaDestinosSinteticos[indiceElegido];
            }
    ...
    manager.exportarJSON("output/output.json", miSilo, globalClock);
